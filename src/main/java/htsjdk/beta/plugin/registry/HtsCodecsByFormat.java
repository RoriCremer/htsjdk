...
//TODO: we need to pass readsDecoderOptions through when resolving bundles to codecs, etc. to ensure
// we use the correct cloudbuffer options, etc.
...
//TODO: we need a way to allow the caller to provide custom codec resolution
...
//TODO: utilize a safe construct for stream inspection (either a separate stream over a fixed byte array,
//      or a safe stream wrapper), to prevent rogue codecs from screwing things up
//TODO: add an entry point that takes codec/version pair to resolve directly
//
// Class used by the registry to track all codecs for a given codec format (i.e., reads).
...
final class HtsCodecsByFormat<F extends Enum<F>, C extends HtsCodec<F, ?, ?>> {
    ...
    public C resolveCodecForInput(
        ...
        final List<C> resolvedCodecs = ioPath.isPresent() ?
                getCodecsForInputIOPath(candidatesForFormat, ioPath.get()) :
    ...
    final List<C> getCodecsForInputIOPath(final List<C> candidateCodecs, final IOPath ioPath) {
        final List<C> uriHandlers = candidateCodecs.stream()
                .filter((c) -> c.claimCustomURI(ioPath))
        ...
        return filteredCodecs.stream()
                ...
                .filter(c -> canDecodeIOPathSignature(c, ioPath))
                .collect(Collectors.toList());
    }
    ...
    private List<C> getCodecsForInputStream(final List<C> candidateCodecs, final BundleResource bundleResource) {
        if (bundleResource.isRandomAccessResource()) {
            ...
            throw new IllegalArgumentException("SeekableStreamResource input  not yet implemented");
        } else {
            // for stream resources, filter on stream signature
            final int streamPrefixSize = candidateCodecs.stream()
                    .map(c -> c.getMinimumStreamDecodeSize())
                    .reduce(0, (a, b) -> Integer.max(a, b));
            final SignatureProbingInputStream signatureProbingStream = bundleResource.getSignatureProbingStream(streamPrefixSize);
            return candidateCodecs.stream()
                    .filter(c -> canDecodeInputStreamSignature(c, streamPrefixSize, bundleResource.getDisplayName(), signatureProbingStream))
                    .collect(Collectors.toList());
        }
    }
    ...
    public static<T extends HtsCodec> boolean canDecodeIOPathSignature(final T codec, final IOPath inputPath) {
        // If the input IOPath has no file system provider, then it must have a custom protocol scheme
        // and probably represents a remote resource. Attempting to get an input stream directly from such
        // an IOPath will likely fail, so fall back to just checking the URI, and let the winning codec access
        ...
        if (!inputPath.hasFileSystemProvider()) {
            ...
            try (final InputStream rawInputStream = inputPath.getInputStream()) {
                return codec.canDecodeSignature(
                        //TODO: the codecs require the input to be buffered
                        // fix this to be smarter...
                        new BufferedInputStream(rawInputStream, 64 * 1024),
    ...
    private static<T extends HtsCodec> boolean canDecodeInputStreamSignature(
            ...
            final InputStream signatureProbingInputStream) {
        ...
        try {
            signatureProbingInputStream.reset(); // for the next guy
...
//    }
    ...
    public Optional<C> getCodecForInputIOPathObsolete(final IOPath inputPath) {
        return getCodecsForInputIOPathObsolete(inputPath)
                .stream()
                .findFirst();
    }

    //TODO: check for/handle > 1 matches ?
    public List<C> getCodecsForInputIOPathObsolete(final IOPath inputPath) {
